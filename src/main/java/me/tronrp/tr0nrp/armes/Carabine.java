...
public class Carabine implements Listener {

    private Main plugin;

    private HashMap<Player, Boolean> tire = new HashMap<>();

    public Carabine(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void onRightClick(PlayerInteractEvent event) {
        ...
        if(it.getType() == Material.GOLDEN_PICKAXE && it.hasItemMeta() && it.getItemMeta().hasDisplayName() && it.getItemMeta().getDisplayName().equalsIgnoreCase("§aCarabine")){
            if(!(tire.containsKey(player)))
                if(player.getInventory().contains(Material.QUARTZ)){
                    ...
                    Vector v = player.getLocation().getDirection().multiply(10D);
                    bullet.setVelocity(v);
                    bullet.setCustomName("Munition lourde");
                    bullet.setShooter(player);
                    suppitem(player,Material.QUARTZ,1);
                    player.getWorld().playSound(player.getLocation(), Sound.BLOCK_TRIPWIRE_CLICK_ON, 3, 1);
    ...
    public void onProjectileHit(ProjectileHitEvent event) {
        ...
        if (projectile.getCustomName() != null && projectile.getCustomName().equals("Munition lourde")) {
            event.setCancelled(true);
            if (hitEntity instanceof LivingEntity) {
                ((LivingEntity) hitEntity).damage(10);
            }
        }
    }
